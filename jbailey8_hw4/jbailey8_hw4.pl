...
/* Problem 1 Answer */
sumlist([], 0).
sumlist([_|T], S) :- S1 is S-1, sumlist(T, S1).
...
sumlist2([], Sum, Sum).
sumlist2([H|T], Ps, S) :- Ps1 is Ps+H, sumlist2(T, Ps1, S).
...
/* Problem 3 Answer */
sumPartialHelper(X, Z, 0, [Z|T]) :- X1 is X-1, sumPartialR(X1, T).
sumPartialHelper(X, Z, M, Y) :- M > 0, M1 is M-1, Z1 is Z+M1, sumPartialHelper(X, Z1, M1, Y).
sumPartialR(0, []).
sumPartialR(X, Y) :- sumPartialHelper(X, X, X, Y).
...
/* Problem 5 Answer: */
/*
   A a) if (x,y) contained in R, we say x is related to y. Here, R is a subset of some Cartesian Product (Y x Z).
     b) a function is a relation for which each value from the set of the first components
	of the ordered pairs is associated with exactly one value from the set of second componenets of the ordered pair
   B) True
   C) False, say we have a relation R such that an element x is related to both a y element and z element, then relation R
      is not a function
*/
...
/* Problem 6 Answer */
outgoing(A,B) :- findall(B, edge(A,B), Out), B = Out.
incoming(A,B) :- findall(B, edge(B,A), Out), B = Out.
...
/*
	Homoiconic means that a statement and data the language processes have no distinction
	
	Prolog is homoiconic

	It means a program can reason about itself, inspecting every aspect of its structure
	and state 

	Prolog is not but it does use reflection mechanisms
*/
...
/* Problem 9 Answer: */
result([],[]).
result([H1|T1], [H2|T2]) :- H2 is H1, result(T1, T2).
...
/* Problem 11 Answer: */
swap(leaf(X), leaf(X)).
swap(tree(X,Y), tree(N,M)) :- swap(N,Y), swap(X,M).
